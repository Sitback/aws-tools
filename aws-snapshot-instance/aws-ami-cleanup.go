...
func cleanupAMI(e *ec2.EC2, cleanupImage ec2.Image) {
	...
	if err != nil || deregisterImageResp.Return != true {
	...
	for blockDevice := range cleanupImage.BlockDevices {
		if len(cleanupImage.BlockDevices[blockDevice].SnapshotId) > 0 {
			...
			ssId := []string{cleanupImage.BlockDevices[blockDevice].SnapshotId}

			_, err := e.DeleteSnapshots(ssId)
			if err != nil {
...
func main() {
	...
	if autoDays > 0 {
		...
		imagesResp, err = e.ImagesByOwners(nil, owners, filter)
		if err != nil {
		...
		imagesResp, err = e.Images(amiIdsSlice, nil)
		if err != nil {
		...
		for tag := range imagesResp.Images[image].Tags {
			if imagesResp.Images[image].Tags[tag].Key == "autocleanup" {
				...
				if int64(autoDays*86000) < amiLifeSpan {
					...
					go func(e *ec2.EC2, anImage ec2.Image) {
						// Decrement the counter when the goroutine completes.
						defer wg.Done()
						// Fetch the URL.
						cleanupAMI(e, anImage)
					}(e, anImage)
